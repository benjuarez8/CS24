...
void *mm_malloc(size_t size) {
    ...
    block_t *coales_block = mm_heap_first;
    while(coales_block <= mm_heap_last && mm_heap_last != NULL) {
        if (!is_allocated(coales_block)) {
            size_t coales_size = get_size(coales_block);
            block_t *head = coales_block;
            coales_block = (void *) coales_block + coales_size;
            while (!is_allocated(coales_block) && coales_block <= mm_heap_last) {
                size_t block_size = get_size(coales_block);
                coales_size = coales_size + block_size;
                coales_block = (void *) coales_block + block_size;
            }
            set_header(head, coales_size, false);
        } else {
            coales_block = (void *) coales_block + get_size(coales_block);
        }
    }
...
void *mm_realloc(void *old_ptr, size_t size) {
    ...
    size_t new_size = get_size(block_from_payload(old_ptr)) - sizeof(block_t);
    if (new_size > size) {
